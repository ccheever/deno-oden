...
fn test_resolve_dns() {
  ...
  async fn run_dns_server(tx: oneshot::Sender<()>) {
    ...
    let authority: Vec<Arc<dyn ZoneHandler>> = vec![
        Arc::new(
          InMemoryZoneHandler::<
            hickory_server::net::runtime::TokioRuntimeProvider,
          >::new(origin, records, ZoneType::Primary, AxfrPolicy::Deny)
          .unwrap(),
        ),
      ];
    ...
    server_fut.register_listener(tcp_listener, Duration::from_secs(2), 32);
