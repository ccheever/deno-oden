...
| compileFunction / evalContext wrappers (user reach) | closed-by-construction | armed Deno[Deno.internal].core is an empty frozen null-prototype facade, so no raw compiler wrapper is reachable |
...
| op_eval_context (Module.wrap / patched _compile) | closed-by-construction | bare Script ids remain unreadable; the armed empty-core facade removes the raw wrapper rather than trusting a caller filename |
