...
impl SessionOverlay {
  ...
  fn can_append_allow(&mut self, selector: &str, req: &Request) -> bool {
    if self.process_requests_blocked {
      return false;
    }
    ...
    self.make_entry_room(selector, true).is_some()
  }
  ...
  fn make_entry_room(&mut self, selector: &str, allow: bool) -> Option<()> {
    ...
    if existing.entries.len() >= MAX_OVERLAY_ENTRIES_PER_PRINCIPAL {
      if allow {
        if !self.tombstone_oldest_allow(Some(selector)) {
          self
            .principals
            .get_mut(selector)
            .expect("principal exists")
    ...
    if self.entry_count >= MAX_OVERLAY_ENTRIES_PER_PROCESS {
      if allow {
        if !self.tombstone_oldest_allow(None) {
          self.process_requests_blocked = true;
        }
      } else {
        self.process_requests_blocked = true;
  ...
  fn tombstone_oldest_allow(&mut self, selector: Option<&str>) -> bool {
    ...
    // @ref llp/0015-dynamic-permissions-with-ceiling.plan.md (Bounded live overlay)
    // Capacity pressure must not remove the only overlay state preventing a
    // covering static floor from reappearing. Convert the evicted allow to a
    // deny in place; the quota-crossing request is refused by make_entry_room.
    self
      .principals
      .get_mut(&selector)
...
mod tests {
  ...
  fn exact_outcomes_compact_and_covering_scans_have_a_constant_ceiling() {
    ...
    let mut bounded = Policy::new(Mode::Enforce);
    bounded.ceiling("dep", "env:read:*", OnRequest::Auto);
    let mut granted_count = 0;
    let mut capacity_count = 0;
    for index in 0..10_000 {
      let request = env(&format!("UNIQUE_{index}"));
      match bounded.evaluate_dynamic_request(&dep("dep"), Some(&request)) {
        DynamicRequestEvaluation::Terminal(DynamicRequestResult {
          state: DynamicPermissionState::Granted,
          ..
        }) => granted_count += 1,
        ...
        other => panic!("unexpected bounded-overlay result: {other:?}"),
      }
    }
    assert_eq!(granted_count, MAX_OVERLAY_ENTRIES_PER_PRINCIPAL);
    assert_eq!(capacity_count, 10_000 - granted_count);
    let session = bounded.session.read().unwrap();
    ...
    drop(session);
    assert_eq!(
      bounded.query_dynamic(&dep("dep"), Some(&env("UNIQUE_0"))),
      DynamicQueryState::Prompt,
      "capacity tombstones narrow authority without widening the floor"
    );
  }
  ...
  fn evicted_reallow_never_restores_a_covering_static_floor() {
    ...
    p.revoke_dynamic(&dep("dep"), &evicted);
    assert!(!p.grants(&dep("dep"), &evicted));
    assert!(matches!(
      p.evaluate_dynamic_request(&dep("dep"), Some(&evicted)),
      DynamicRequestEvaluation::Terminal(DynamicRequestResult {
        state: DynamicPermissionState::Granted,
        ..
      })
    ...
    for index in 0..(MAX_OVERLAY_ENTRIES_PER_PRINCIPAL - 1) {
      let request = env(&format!("FILLER_{index}"));
      assert!(matches!(
    ...
    let crossing = env("QUOTA_CROSSING");
    assert!(matches!(
      p.evaluate_dynamic_request(&dep("dep"), Some(&crossing)),
      DynamicRequestEvaluation::Terminal(DynamicRequestResult {
        state: DynamicPermissionState::Denied,
        code: DynamicRequestCode::Capacity,
        ..
    ...
    assert!(
      !p.grants(&dep("dep"), &evicted),
      "an evicted re-allow must remain a deny tombstone instead of revealing the static floor"
    );
    assert_eq!(
      p.query_dynamic(&dep("dep"), Some(&evicted)),
      DynamicQueryState::Prompt
    );
    assert!(matches!(
      p.evaluate_dynamic_request(&dep("dep"), Some(&evicted)),
  ...
  #[test]
  fn quota_crossing_is_capacity_before_an_external_decider() {
    let mut p = Policy::new(Mode::Enforce);
    p.ceiling("dep", "env:read:*", OnRequest::Prompt);

    for index in 0..MAX_OVERLAY_ENTRIES_PER_PRINCIPAL {
      let request = env(&format!("PROMPT_FILLER_{index}"));
      let DynamicRequestEvaluation::NeedsDecision { ceiling } =
        p.evaluate_dynamic_request(&dep("dep"), Some(&request))
      else {
        panic!("room below the quota should reach the external decider")
      };
      assert_eq!(
        p.complete_dynamic_request(
          &dep("dep"),
          &request,
          ceiling,
          DynamicDecider::Broker,
          true,
        )
        .state,
        DynamicPermissionState::Granted
      );
    }

    assert!(matches!(
      p.evaluate_dynamic_request(
        &dep("dep"),
        Some(&env("NO_PROMPT_AT_CAPACITY"))
      ),
      DynamicRequestEvaluation::Terminal(DynamicRequestResult {
        state: DynamicPermissionState::Denied,
        code: DynamicRequestCode::Capacity,
      ...
      .map(|worker| {
        ...
        std::thread::spawn(move || {
          ...
          for index in 0..1_000 {
            ...
            let result = p.evaluate_dynamic_request(&principal, Some(&request));
            if index < MAX_OVERLAY_ENTRIES_PER_PRINCIPAL {
              assert!(matches!(
                result,
                DynamicRequestEvaluation::Terminal(DynamicRequestResult {
                  state: DynamicPermissionState::Granted,
